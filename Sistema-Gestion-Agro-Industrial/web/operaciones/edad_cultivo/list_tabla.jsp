...
<table id="tabla" class="table table-striped location_table">
    ...
    <tbody>                                                                                
        <tr>
            ...
            <td>
                <a href="javascript:void(0)" onclick="edit_form(<%=entidad.getId_edad_cultivo()%>,'<%=entidad.getNombre()%>','<%=entidad.getDescripcion()%>','<%=entidad.getUsuario_responsable()%>','<%=entidad.getEstado()%>')" class="comp_edit btn btn-primary btn-mini">Editar</a>
