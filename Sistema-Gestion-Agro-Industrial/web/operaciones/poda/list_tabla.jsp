...
List<entPoda> list=clsGestor.ListarPoda(false);
if(list!=null)
{%>
<div id="frame">
 <button class="btn btn-invert" onclick="tablaTemp(0)" type="button">Agregar Poda</button>
<div class="row-fluid">
...
<table id="lista" class="table table-striped location_table">
    <thead>
     ...
     <tbody>      
    <%
    for(entPoda entidad : list)
    {
    %>
                                                                   
        <tr>
            <td><%=entidad.getId_poda()%></td>
            ...
            <td>
                <a href="javascript:void(0)" onclick="editar(<%=entidad.getId_poda()%>)" class="comp_edit btn btn-primary btn-mini">Ver o Editar</a>
            </td>
        </tr>
...
<%}
    ...
    out.print("<button class='btn btn-invert' onclick='tablaTemp(0)' type='button'>Agregar Poda</button>");
}}%>  
