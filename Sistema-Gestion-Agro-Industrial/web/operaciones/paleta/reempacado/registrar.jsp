...
$('#grabar').click(function(){
    if($('input#IdPaleta').val()!='' || $('input#IdPaleta').val()!=null){
         smoke.confirm('Desea Reempacar esta Paleta',function(e){
            if (e){
                var url = "operaciones/paleta/reempacado/insert.jsp"; 
                $.ajax({
                type: "POST",
                url: url,
...
function compruebaData(data)
{
   ...
   for (i = 0; i <=maxI; i++) { 
        ...
        {  
            ...
            $.ajax({
                url: 'operaciones/paleta/reempacado/cargar.jsp?codigo='+paleta.entidad[i].codigo_control,
                type: 'POST',
                success: function (data) {   
                        $('#tabla').html(data); 
                        $('#grabar').show();
