...
<table id="lista" class="table table-striped location_table">
    ...
    %>
        ...
        <tr>
            ...
            <td>
                <a href="javascript:void(0)" onclick="edit_form(<%=entidad.getId_color()%>,'<%=entidad.getNombre()%>','<%=entidad.getCodigo_control()%>',<%=entidad.getObjCultivo().getId_cultivo()%>,'<%=entidad.getObjCultivo().getNombre()%>','<%=entidad.isEstado()%>')" class="comp_edit btn btn-primary btn-mini">Editar</a>
