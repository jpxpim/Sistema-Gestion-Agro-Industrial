...
if(objSession!=null)
{
    ...
    int posJ=objSession.getListModulos().get(objSession.getPosicion()).getList().size();
        ...
        {
            if(89==objSession.getListModulos().get(objSession.getPosicion()).getList().get(j).getControl_form())
            {
...
function tabla(operacion)
{
    $('#tabla').html('<center id="tabla"><h3><img src="img/ajax-loader.gif" alt="" /> Espere un Momento ...</h3></center>');
     $.ajax({
        url: 'operaciones/reporte/cajas_seleccionador_trabjador.jsp?operacion='+operacion,
        type: 'POST',
        success: function (data) {     
                 $('#tabla').html(data);
        },
