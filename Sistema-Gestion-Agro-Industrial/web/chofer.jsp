...
if(objSession!=null)
{
    ...
    int posJ=objSession.getListModulos().get(objSession.getPosicion()).getList().size();
        ...
        {
            if(42==objSession.getListModulos().get(objSession.getPosicion()).getList().get(j).getControl_form())
            {
...
{
     $.ajax({
        url: 'operaciones/chofer/list_tabla.jsp',
        type: 'POST',
        success: function (data) {     
                 $('#tabla').html(data);
        },
   ...
   function getReporte()
{
     window.location='operaciones/chofer/reporte.jsp';
 };
 function comboTransportista()
{
     $.ajax({
        url: 'operaciones/transportista/list_combo.jsp',
        type: 'POST',
        success: function (data) {     
                 $('#cbTransportista').html(data);
        },
        contentType: false,
        processData: false
    });          
					...
					errorPlacement: function(error, element) {
						$(element).closest('div').append(error);
                                    ...
                                    function clear_form() {
                                            ...
                                            if(comboId>0)
                                              $("#cbTransportista option[value='"+comboId+"']").remove();
                                           $("select#cbTransportista").val(0);    
                                            $("#IdChofer").val("0");  
                                           
                                      };
                                       function edit_form(id,nombre,brevete,idTransportista,nTransportista,estado) {
                                            
                                            if(comboId>0)
                                              $("#cbTransportista option[value='"+comboId+"']").remove();
                                            $('#txtBrevete').val(brevete);
                                            ...
                                            if(buscaCombo(idTransportista))
                                               $("select#cbTransportista").val(idTransportista); 
                                            else
                                            {
                                                comboId=idTransportista;
                                                $("#cbTransportista").append('<option value='+idTransportista+'>'+nTransportista+'</option>');
                                                $("select#cbTransportista").val(idTransportista); 
                                            }
                                       ...
                                       function buscaCombo(valor) {
                                           var estado=false;
                                            $("#cbTransportista option").each(function(){
